...
"""
SCRIPT PARA GERAÇÃO DE TOPOLOGIAS DE REDE PARA OPERADORAS NACIONAIS DE TELECOM
"""
...
"""
    ...
    # 1. Gerar RTICs (hubs estratégicos)
    for regiao, dados in REGIOES_HIERARQUIA.items():
        for hub in dados["hubs"]:
            # Encontra a cidade hub na lista
            cidade_hub = next((c for c in todas_cidades if c[0] == hub), None)
            if cidade_hub:
                siteid = gerar_siteid(
    ...
    # 2. Gerar RTRRs (sub-regiões)
    for regiao, dados in REGIOES_HIERARQUIA.items():
        for sub_regiao, ufs in dados["sub-regioes"].items():
            # Selecionar uma cidade representativa (capital da sub-região)
            # Usamos a primeira UF da lista e a primeira cidade dessa UF
            uf_rep = ufs[0]
            cidades_sub = [c for c in cidades_por_regiao[regiao] if c[1] == uf_rep]
            if cidades_sub:
    ...
    # Atualizar distribuição real de RTIC e RTRR
    dist_real = {
        "RTIC": len(rtics),
        "RTRR": len(rtrrs),
    }
    ...
    total_restante = args.e - dist_real["RTIC"] - dist_real["RTRR"]
    proporcao_restante = {
        "RTPR": PROPORCAO_CAMADAS["RTPR"] / (PROPORCAO_CAMADAS["RTPR"] + PROPORCAO_CAMADAS["RTED"] + PROPORCAO_CAMADAS["SWAC"]),
        "RTED": PROPORCAO_CAMADAS["RTED"] / (PROPORCAO_CAMADAS["RTPR"] + PROPORCAO_CAMADAS["RTED"] + PROPORCAO_CAMADAS["SWAC"]),
        "SWAC": PROPORCAO_CAMADAS["SWAC"] / (PROPORCAO_CAMADAS["RTPR"] + PROPORCAO_CAMADAS["RTED"] + PROPORCAO_CAMADAS["SWAC"])
    }
    
    dist_real["RTPR"] = max(2, round(total_restante * proporcao_restante["RTPR"]))
    dist_real["RTED"] = max(2, round(total_restante * proporcao_restante["RTED"]))
    # Garantir que RTED seja par
    if dist_real["RTED"] % 2 != 0:
        dist_real["RTED"] += 1
    dist_real["SWAC"] = total_restante - dist_real["RTPR"] - dist_real["RTED"]
